...
class MainWindow(QMainWindow):
    ...
    def load_data(self):
        """Gets data from database and puts it into rows and columns"""
...
class InsertRecord(QDialog):
    ...
    def add_student(self):
        """Adds data to students table in database"""
        name = self.student_name.text()
        course = self.course_name.itemText(self.course_name.currentIndex())
        phone_num = self.phone_number.text()
        connection = DatabaseConnection().connect()
        cursor = connection.cursor()
        cursor.execute("INSERT INTO students (name, course, mobile) VALUES (%s,%s,%s)", (name, course, phone_num))
